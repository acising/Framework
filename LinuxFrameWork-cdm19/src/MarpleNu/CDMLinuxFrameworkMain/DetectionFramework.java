...
public class DetectionFramework {
    ...
    private static HashMap<String, DetectionFramework> clientId2Instance = new HashMap<>();

    private static HashMap<String, Integer> judgeMode2AlertLevel = new HashMap<>();
    ...
    private ArrayList<ArrayList<ArrayList<String>>> transGrammar = new ArrayList<>();	//transmit grammar structure
    private ArrayList<ArrayList<String>> labelGrammar = new ArrayList<>();	//label grammar structure
    private LinkedHashMap<String, ArrayList<ArrayList<String>>> judgeGrammar = new LinkedHashMap<>();	//judge output grammar structure
    private ArrayList<ArrayList<String>> eventGrammar = new ArrayList<>();	//event structure

    private ConcurrentHashMap<String, ConcurrentHashMap<Integer, ArrayList<ProcessNode>>> ProcessHangingPortList = new ConcurrentHashMap<>(); //record the s-process who link to this d-port but we don't know who listen to this d-port, when get the d-process from the PortList, we clear this list.
    private ConcurrentHashMap<String, ConcurrentHashMap<Integer, ProcessNode>> PortList = new ConcurrentHashMap<>(); //record the process who listens to the port

    private ConcurrentHashMap<String, HashSet<String>> IpList = new ConcurrentHashMap<>();
    private ConcurrentHashMap<String, ConcurrentHashMap<String, FileNode>> FileList = new ConcurrentHashMap<>(); //the file list
    private ConcurrentHashMap<String, ConcurrentHashMap<Integer, ProcessNode>> ProcessList = new ConcurrentHashMap<>(); //use the process uid as key, only consider the process which is on.
    private ConcurrentHashMap<String, ProcessNode> ProcessRoot = new ConcurrentHashMap<>();//ProcessNode("", 0, 0, 0);   //the root of the process tree.
    private ConcurrentHashMap<String, ConcurrentHashMap<Long, ProcessNode>> TotalStartProcess = new ConcurrentHashMap<>();
    ...
    private HashMap<Integer,List<String>> outputList = new HashMap<Integer,List<String>>();
        ...
        if(event.eventType == 2||event.eventType==3||event.eventType==13){
            ...
            if (event.eventType==2) {
                for (Object o : parentNode.labelList.entrySet()) {
                    ...
                    if(((String)key).equals("PT10")||
                            ((String)key).equals("PT9")||
                            ((String)key).equals("PT4")||
                            ((String)key).equals("PT5")||
                            ((String)key).equals("PT6")){
                        continue;
                    }
            ...
            if(Node == null){
                ...
                if(parentNode==null){
                    ...
                    for (Object o : parentNode.labelList.entrySet()) {
                        Map.Entry entry = (Map.Entry) o;
                        Object key = entry.getKey();
                        if (((String) key).equals("PT7") ||
                                ((String) key).equals("PT8") ||
                                ((String) key).equals("PT4")) {
                            Object val = entry.getValue();
                            LabelForCS labelForCS = new LabelForCS((LabelForCS) val);
